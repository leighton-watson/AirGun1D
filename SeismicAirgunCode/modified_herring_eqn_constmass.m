function [dYdt] = modified_herring_eqn_constmass(t, Y, params)
...
dQdt = 4*pi*M*kappa*R^2*(T_bubble - T_infty);
...
%%%BUBBLE MASS%%%
if (t < t_fire)%&& (p_airgun > p_bubble); %airgun is firing
    dm_bubble_dt = dmdt;
else
    dm_bubble_dt = 0;
end
...
%%%BUBBLE VELOCITY%%%
alpha = 0.8;
dU_dt = (1/R)*(1/rho_infty*(p_bubble-p_infty+pv + R*dp_bubble_dt/c_infty) - 3/2*U^2 - alpha*U);

%% EVOLVE SOLUTION %%

dYdt = [dR_dt; dU_dt; dm_bubble_dt; dT_bubble_dt;...
    dp_airgun_dt; dm_airgun_dt; 0];