classdef DiscrAirgun_initBubbleVol < noname.Discretization
    properties
    ...
    methods
        function obj = DiscrAirgun_initBubbleVol(m,order,initBubbleVol,airgunPressure, airgunLength, airgunPortArea, airgunDepth)
            default_arg('m',100)
            default_arg('order',4)

            [physConst, t0, icAirgun, icBubble] = configAirgun_initBubbleVol('Bolt1500LL',initBubbleVol,airgunPressure, airgunLength, airgunPortArea, airgunDepth);
            V_bubble = initBubbleVol * 1.63871e-5; % initial bubble volume (m^3)
            icBubble.R = (3/(4*pi) * V_bubble)^(1/3);
            icBubble.m = physConst.p_inf*V_bubble / (physConst.R_G*physConst.Tinf);
