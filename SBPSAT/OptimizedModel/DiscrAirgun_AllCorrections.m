classdef DiscrAirgun_AllCorrections < noname.Discretization
    properties
    ...
    methods
        function obj = DiscrAirgun_AllCorrections(m,order,airgunPressure,airgunLength,airgunPortArea,airgunDepth, dissp_boolean, beta, Mfac, Vinit)
            default_arg('m',100)
            default_arg('order',4)

            %[physConst, t0, icAirgun, icBubble] = configAirgun('shocktube');
            %[physConst, t0, icAirgun, icBubble] = configAirgun('nonphys');
            [physConst, t0, icAirgun, icBubble] = configAirgun_AllCorrections('Bolt1500LL',airgunPressure,airgunLength,airgunPortArea,airgunDepth, Vinit);
            ...
            function [dq, dBubble] = RHS(q,t,bubble)
                ...
                if t >= physConst.AirgunCutoffTime || flowState == scheme.Euler1d.SUBSONIC_INFLOW
                    dq = q.*0;
                    dBubble = bubbleRHS_AllCorrections(bubble, 0, 0, 0, 0, 0, physConst, dissp_boolean, beta, Mfac);
                    return
                end
                ...
                e_a = qHat(3);
                p_a = schm.p(qHat);
                dBubble = bubbleRHS_AllCorrections(bubble, rho_a, v_a, e_a, p_a, A, physConst, dissp_boolean, beta, Mfac);
